...
| `merge_record` | `survivor_id`, `merged_id`, `evidence text`, `evidence_refs text[]`, `actor_kind`, `actor_id`, `merged_at`, `unmerged_at null`, `unmerged_by_kind null`, `unmerged_by_id null`, `unmerge_note text null` | Names the evidence, the actor, and the time (R4). One row per merge; an unmerge fills the `unmerged_*` columns and never deletes the row. |
...
- Merge records naming a removed party keep their identifiers and timestamps and lose nothing
  else, because they hold no content; review candidates naming it close as `withdrawn`.
